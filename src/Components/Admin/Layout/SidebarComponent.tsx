                            ...
                            return (
                                ms?.child ? <div key={i}>
                                    <SidebarItem
                                        ...
                                        Icon={ms?.Icon} label={ms?.label} href={ms?.href} child={true} isActive={String(route?.pathname) === `/admin${ms?.href}` ? true : false}
                                    >
                                    ...
                                    <div className={`${isOpen ? 'block' : 'hidden'} pl-8 pt-1 pb-1 space-y-1`}>
                                        {
                                            ms?.child?.map((c, i) => (
                                                <button key={i} onClick={() => route?.push(`/admin${ms?.href}${c?.href}`)} className={`block p-2 w-full text-left rounded-lg text-sm ${route?.pathname === `/admin${ms?.href}${c?.href}` ? 'text-blue-800 font-bold bg-blue-200' : 'text-gray-600'} hover:bg-gray-100 transition duration-150`}>{c?.label}</button>
                                            ))
                                        }
                                    </div >
                                </div> :
                                    <SidebarItem key={i} Icon={ms?.Icon} label={ms?.label} href={ms?.href} isActive={String(route?.pathname) === `/admin${ms?.href}` ? true : false} />
                            )
                        })
                    }
                </nav>
