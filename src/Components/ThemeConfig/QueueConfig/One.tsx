    ...
    return (
        <div className={`${color?.bg100} p-4 rounded-xl shadow-md mb-4 flex flex-col sm:flex-row justify-between items-center space-y-2 sm:space-y-0 ${color?.text800}`}>
            ...
            <div className='flex space-x-4 text-sm font-medium'>
                <span>Antrian Pesanan: <span className={`font-bold ${color?.text600}`}>{10}</span></span>
                <span>Antrian Sekarang: <span className={`font-bold ${color?.text600}`}>{10 > 0 ? 3 : 0}</span></span>
            </div>
        </div>
    )
}
