...
const Six = ({ color, categories, isDarkMode }: Props) => {
    return (
        <section>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {categories.map((cat, i) => (
                    <div key={i} className="relative h-64 rounded-2xl overflow-hidden group">
                        <img src={cat.image} className={`absolute inset-0 w-full h-full object-cover grayscale ${isDarkMode && "bg-white"} group-hover:grayscale-50 `} alt="" />
