...
const One = ({ color, categories }: Props) => {
    return (
        <section>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 h-[500px]">
                <div className="md:col-span-2 md:row-span-2 relative group overflow-hidden rounded-3xl">
                    <img src={categories[0].image} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-8 flex flex-col justify-end">
                        <h3 className="text-3xl font-bold text-white">{categories[0].name}</h3>
                        <p className="text-white/60">{categories[0].count}</p>
                    </div>
                </div>
                {categories.slice(1).map((cat, i) => (
                    <div key={i} className="relative group overflow-hidden rounded-3xl">
                        <img src={cat.image} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="" />
