...
import CardProductTherty from './Card/CardProductTherty';
...
const ListProductTwelve = ({ filteredProducts, openDetailModal, handleToggleFavorite, color, activeCategory }: Props) => {
    return (
        <>
            {
                ...
                <section className="mb-12">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredProducts?.filter((f) => !f?.isPackage).map(product => (
                            <CardProductTherty key={product.id}
                                product={product}
...
export default ListProductTwelve