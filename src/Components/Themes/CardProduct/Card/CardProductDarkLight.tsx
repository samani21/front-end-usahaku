...
const CardProductDarkLight = ({ product, onClick, onToggleFavorite, color, themeMode }: Props) => {
    const addHover = (cls?: string) => cls ? cls.replace(/^(.*?)-/, 'hover:$&') : "";
            ...
            <div className="flex-1 p-1 flex flex-col justify-between">
                <div>
                    <div className="flex justify-between items-center">
                        ...
                        {product.isFavorite ? (
                            <HeartIcon className="text-red-500 w-5 h-5 flex-shrink-0" onClick={() => onToggleFavorite(product?.id)} />
                        ) : <HeartIcon className="text-gray-500 w-5 h-5 flex-shrink-0" onClick={() => onToggleFavorite(product?.id)} />}
                    </div>
...
export default CardProductDarkLight