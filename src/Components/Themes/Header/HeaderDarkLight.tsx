        ...
        <header className={`sticky top-0 z-30 ${bgColor} ${shadow}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
                ...
                <div className="flex space-x-5 items-center">
                    ...
                    <div onClick={() => openDrawer('favorite')} title="Favorit">
                        <HeartIcon className="text-red-500 hover:text-red-700 transition transform hover:scale-110" onClick={() => openDrawer('favorite')} />
                    </div>
                    <div onClick={() => openDrawer('cart')} title="Pesanan Saat Ini">
                        <ShoppingCartIcon className={`${primaryColor} hover:opacity-80 transition transform hover:scale-110`} onClick={() => openDrawer('cart')} />
                    </div>
                    <div onClick={() => openDrawer('history')} title="Riwayat Pesanan">
                        <HistoryIcon className={`${secondaryColor} hover:opacity-80 transition transform hover:scale-110`} onClick={() => openDrawer('history')} />
                    </div>
                </div>
            </div>
        </header>
