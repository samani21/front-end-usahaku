...
const HeaderDarkLight: React.FC<HeaderProps> = ({ onThemeToggle, openDrawer, favoriteProducts, cart, history, color, themeMode, header }) => {
    ...
    return (
        <>
            ...
            <nav className={`flex sm:hidden ${bgColor} fixed bottom-0 w-full justify-between px-8 pt-1`}>
                <div onClick={onThemeToggle} title={themeIconTitle} className={`p-2 rounded-full text-gray-600 transition duration-150 relative group`}>
                    <ThemeIcon className={`${themeIconColor} hover:opacity-80 transition transform hover:scale-110`} />
                </div>
