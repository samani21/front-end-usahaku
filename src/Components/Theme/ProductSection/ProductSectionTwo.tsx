...
const ProductSectionTwo = ({ filteredProducts, activeCategory, onClick, color, handleFav }: Props) => {
    return (
        <div className="mt-10 px-4 sm:px-6 lg:px-8" id="produk-pilihan">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                {filteredProducts.map(product => (
                    <ProductCardTwo key={product.id} product={product} color={color} onClick={(p) => onClick(p)} handleFav={handleFav} />
                ))}
