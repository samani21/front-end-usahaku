import { ThemeColor } from '@/lib/Types/Theme/Six';
import { Clock, Heart, ShoppingCart, X } from 'lucide-react';
import React from 'react'

const IconDrawer: React.FC<{ isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode; theme: ThemeColor }> =
    ({ isOpen, onClose, title, children, theme }) => {
        return (
                ...
                <div className={`fixed right-0 top-0 h-full w-full max-w-sm bg-white shadow-2xl p-6 transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'} rounded-l-2xl`}>
                    <div className="flex justify-between items-center pb-4 border-b border-gray-100">
                        <h2 className="text-xl font-bold text-gray-800 flex items-center">
                            {title === "Pesanan Anda" ? <ShoppingCart className={`w-5 h-5 mr-2 text-${theme.primary}-500`} /> :
                                title === "Riwayat Pesanan" ? <Clock className={`w-5 h-5 mr-2 text-${theme.primary}-500`} /> :
                                    <Heart className={`w-5 h-5 mr-2 text-${theme.primary}-500`} />}
                            {title}
                        </h2>
