...
const SideDrawerOne: React.FC<SideDrawerOneProps> = ({ isOpen, onClose, title, children }) => {
    ...
    return (
        <>
            ...
            <div
                className={`fixed right-0 top-0 h-full w-full md:w-96 bg-gray-800 shadow-2xl shadow-black/50 z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
                role="dialog"
                aria-modal="true"
            ...
            >
                <div className={`flex justify-between items-center p-4 border-b border-gray-700/20 ${headerBg} text-white`}>
                    ...
                    <XIcon className="text-white hover:text-white/80 cursor-pointer" onClick={onClose} />
                </div>
